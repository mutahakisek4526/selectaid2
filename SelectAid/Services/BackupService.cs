...
public class BackupService
{
    ...
    public void RestoreBackup(string zipPath)
    {
        ...
        var backupDir = Path.Combine(_dataRoot, "backups");
        Directory.CreateDirectory(backupDir);
        var safetyPath = Path.Combine(backupDir, $"pre-restore-{DateTime.Now:yyyyMMdd-HHmmss}.zip");
        if (!File.Exists(safetyPath))
        {
            ZipFile.CreateFromDirectory(_dataRoot, safetyPath, CompressionLevel.Optimal, true);
        }
